...
- `orders` — Order headers with status, pricing, auth/capture amounts, delivery details, payment method
- `order_items` — Line items per order (catalog or shopping list type, with fulfillment fields)
- `order_flags` — Admin review queue for payment issues (capture failures, overages, cash discrepancies)
