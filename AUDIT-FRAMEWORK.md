...
### 6.4 Design Standards Compliance

Check for:

- [ ] **No emojis in the UI.** Spec mockups use emojis as shorthand, but the implementation must replace them with SVG icons or text labels. Emojis render inconsistently and look unprofessional.
- [ ] **No AI-generated copy patterns.** No em dashes, no "not X, but Y" constructions, no throat-clearing phrases ("It's worth noting..."), no semicolons in customer-facing text.
- [ ] **Copy is direct and plain-language.** Short sentences. Says what it IS, not what it isn't. Appropriate for rural Maine, median age 48, mixed tech literacy.
- [ ] **Color palette is consistent.** Primary blue (#1A56DB or the decided shade) for CTAs and accents. No random accent colors. No gradients.
- [ ] **Error messages are specific and helpful.** Not generic ("An error occurred") and not technical ("Validation error: field X").
- [ ] **Buttons and interactive elements are clearly tappable.** 44x44px minimum touch target. Primary action is visually prominent (solid blue fill). Secondary actions are less prominent (outlined or ghost).
